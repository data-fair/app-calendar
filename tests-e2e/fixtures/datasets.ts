...
const schemas = schemasJson as unknown as Record<string, { id: string; title: string; finalizedAt: string; schema: any[]; isRest?: boolean; userPermissions?: string[] }>
...
export const datasets: Record<DatasetKey, TestDataset> = Object.fromEntries(
  (Object.keys(schemas) as DatasetKey[]).map((key) => {
    ...
    return [key, {
      ...
      schema: ds.schema,
      isRest: ds.isRest,
      userPermissions: ds.userPermissions,
    }]
  })
...
export function makeDatasetEntry (key: DatasetKey) {
  ...
  return {
    ...
    finalizedAt: ds.finalizedAt,
    schema: ds.schema,
    ...(ds.isRest ? { isRest: true } : {}),
    ...(ds.userPermissions ? { userPermissions: ds.userPermissions } : {}),
  }
}
