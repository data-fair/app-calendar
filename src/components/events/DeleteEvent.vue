...
const { mainDataset, contribsDataset } = useAppInfo()
...
async function deleteEvent () {
  const url = `${prop.event.isContrib ? (contribsDataset.href + `/own/${prop.event._owner}`) : mainDataset.href}/lines/${prop.event.id}`
  const params = {
    method: 'DELETE'
  }
  try {
